...
async function goBot(dado) {
    ...
    for await(ip of dado) {
        ...
        try {
            ...
            let url = await `https://www.abuseipdb.com/check/${ip}` ;
            await page.goto(url) ; 

            ipState =   await `<b>${ip}</b> was not found in our database` ;    
            ...
            await (async () => {
                ...
                if(stateIp.indexOf(ipState) > -1){
                    await personalize.setWorksheet(ip, isp, url) ;
                    await console.log(`${count}->  IP: ${ip}   -  Checado!`) ;
                }
                else {  
                    porcent = await page.evaluate(body => document.querySelector(".progress span").innerHTML);
                    await console.log(`${count}->  IP: ${ip}   -  Checado!`);
                    await personalize.setWorksheetBad(ip, isp, porcent, url) ;
                }
            })();
        
        }catch(err) {
            console.log(`${count}->  IP: ${ip}  -  Privado ou Invalido`);
        }
